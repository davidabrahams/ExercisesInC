...
float my_random_float2()
{
  ...
  // What if this runs more than 8 times? Happens 1/256 times...
  // seems like sometimes part of the random number gets reused.
  while (x & mask) {
    mask <<= 1;
    exp--;
  }
...
double my_random_double()
{
  long x;
  long mant;
  long exp = 1022; // 2^(11-1) - 2 is an exponent of -1, b/c bias is 2^10 - 1
  long mask = 1;
  ...
  // generate random bits until we see the first set bit
  while (1) {
    x = ((long) random() << 32) | random(); // generate a random 64-bit long
    if (x == 0) {
      exp -= 31;
    } else {
      break;
