...
Yes, but it is not guaranteed to be (because the threads run concurrently)
...
Yes. Running the program multiple times produces inconsistent output. For example:

```
➜  ex12 git:(master) ./counter
counter = 0
counter = 0
counter = 1
counter = 0
...
Apparently a lot. It took `0.19s`, as opposed to `0.03s`.
